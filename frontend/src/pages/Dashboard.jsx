...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [clientsRes, videosRes, paymentsRes] = await Promise.all([
        supabase.from('clients').select('*'),
        supabase.from('videos').select('*, clients(name)').eq('year', year).eq('month', month),
      ...
      const pending = videos.filter((v) => v.editor_status !== 'Done' || (v.client_status && ['Correction','Rejected'].includes(v.client_status)));
      const awaitingClient = videos.filter((v) => v.editor_status === 'Done' && !v.client_status).length;

      const billing = videos.filter((v) => ['Posted','Approved'].includes(v.client_status)).reduce((a,v) => a + Number(v.amount || 0), 0);
  ...
  if (loading) {
    return <div className="py-20 flex justify-center"><Loader2 className="w-6 h-6 text-[#2dd4bf] animate-spin" /></div>;
  }
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {pendingVideos.length === 0 ? (
                <tr><td colSpan={7} className="px-5 py-10 text-center text-[#7c8d8e]">No pending videos this month.</td></tr>
              ) : pendingVideos.map((v) => (
                <tr key={v.id} className="border-b border-[#101a1b] last:border-b-0 hover:bg-[#0f1819] transition-colors">
                  <td className="px-5 py-3.5">
                    <Link to={`/admin/clients/${v.client_id}`} className="text-[#2dd4bf] hover:underline">{v.clients?.name}</Link>
                  </td>
                  <td className="px-5 py-3.5 text-[#d6e7e6]">{v.name}</td>
                  <td className="px-5 py-3.5 text-[#9bb0b1]">{MONTH_LABEL[v.month-1]} {v.year}</td>
      ...
      <section>
            ...
            <tbody>
              {payments.length === 0 ? (
                ...
                <tr key={p.id} className="border-b border-[#101a1b] last:border-b-0 hover:bg-[#0f1819] transition-colors">
                  <td className="px-5 py-3.5">
                    <Link to={`/admin/clients/${p.client_id}`} className="text-[#2dd4bf] hover:underline">{p.clients?.name}</Link>
                  </td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
          <div className="rounded-xl border border-[#142021] bg-[#0d1516] px-5 py-4 text-center">
            ...
            <div className="text-2xl font-bold font-mono-num text-[#e6f7f6]">₹{totalBilling.toLocaleString()}</div>
          </div>
          <div className="rounded-xl border border-[#142021] bg-[#0d1516] px-5 py-4 text-center">
            <div className="text-[12px] text-[#7c8d8e] mb-1">Received</div>
            <div className="text-2xl font-bold font-mono-num text-emerald-400">₹{received.toLocaleString()}</div>
          </div>
          <div className="rounded-xl border border-[#142021] bg-[#0d1516] px-5 py-4 text-center">
            <div className="text-[12px] text-[#7c8d8e] mb-1">Pending</div>
            <div className="text-2xl font-bold font-mono-num text-rose-400">₹{pendingTotal.toLocaleString()}</div>
          </div>
        </div>
      </section>
    </div>
