...
import { fetchClient, fetchClientVideos, EDITOR_STATUSES, CLIENT_STATUSES, setEditorStatus, setClientStatus, fetchCorrections } from "../lib/api";
...
export default function ClientPortalDetail() {
  ...
  const load = async () => {
    setLoading(true);
    try { const [c, vs, cors] = await Promise.all([fetchClient(id), fetchClientVideos(id), fetchCorrections({ clientId: id })]);
  ...
  const onEditor = async (v, val) => { try { await setEditorStatus(v.id, val); toast.success(`Editor → ${val}`); load(); } catch (e) { toast.error(e.message); } };
  const onClient = async (v, val) => { try { await setClientStatus(v.id, { client_status: val, posted_date: val === 'Approved' ? v.posted_date : null }); toast.success(`Client → ${val}`); load(); } catch (e) { toast.error(e.message); } };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-7">
      ...
      <div className="rounded-2xl border border-[#142021] bg-[#0d1516] px-6 py-5">
        <div className="flex items-center gap-5"><Avatar name={client.name} size={56} className="text-xl" />
          <div>
            ...
            {videos.map((v) => (
              <div key={v.id} className="rounded-xl border border-[#142021] bg-[#0d1516] px-5 py-4">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="w-12 h-12 rounded-lg bg-[#0e2624] border border-[#143a37] flex items-center justify-center shrink-0"><Play className="w-5 h-5 text-[#2dd4bf]" fill="currentColor" /></div>
                  ...
                  <Select value={v.editor_status} onValueChange={(val) => onEditor(v, val)}>
                    <SelectTrigger className="w-[170px] bg-[#0a1112] border border-[#243334] text-[#e6f7f6] h-9"><SelectValue /></SelectTrigger>
                  ...
                  <Select value={v.client_status || 'Pending Review'} onValueChange={(val) => onClient(v, val)}>
                    <SelectTrigger className="w-[160px] bg-[#0a1112] border border-[#243334] text-[#e6f7f6] h-9"><SelectValue /></SelectTrigger>
      ...
      {corrections.length > 0 && (
        <section><h2 className="text-lg font-bold mb-3">Corrections</h2>
