...
import { createCorrection, setClientStatus } from '../lib/api';
...
export default function CorrectionForm({ open, onOpenChange, video, clientId, onSubmitted }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await setClientStatus(video.id, { client_status: 'Correction', posted_date: null });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#0d1516] border border-[#142021] text-[#e6f7f6] max-w-lg">
        ...
        <form onSubmit={submit} className="space-y-3">
          <label className="block"><span className="block text-[12px] text-[#8aa0a1] mb-1.5">Correction Title</span>
            <input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g., Audio sync off at 0:12"
              className="w-full px-3 py-2 rounded-lg bg-[#0a1112] border border-[#243334] text-[#e6f7f6] outline-none focus:border-[#2dd4bf] focus:ring-2 focus:ring-[#2dd4bf]/20" /></label>
          <label className="block"><span className="block text-[12px] text-[#8aa0a1] mb-1.5">Description</span>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Describe the change needed…"
              className="w-full px-3 py-2 rounded-lg bg-[#0a1112] border border-[#243334] text-[#e6f7f6] outline-none focus:border-[#2dd4bf] focus:ring-2 focus:ring-[#2dd4bf]/20" /></label>
          <label className="block"><span className="block text-[12px] text-[#8aa0a1] mb-1.5">Priority</span>
            <Select value={priority} onValueChange={setPriority}>
              <SelectTrigger className="bg-[#0a1112] border border-[#243334] text-[#e6f7f6] h-[38px]"><SelectValue /></SelectTrigger>
              <SelectContent className="bg-[#0d1516] border border-[#243334] text-[#e6f7f6]">{['Low','Medium','High','Urgent'].map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
            </Select>
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><span className="block text-[12px] text-[#8aa0a1] mb-1.5">Screenshot (optional)</span>
            ...
            <label className="block"><span className="block text-[12px] text-[#8aa0a1] mb-1.5">Voice Note (optional)</span>
              <label className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-[#243334] bg-[#0a1112] text-[#8aa0a1] text-[12px] cursor-pointer hover:border-[#2dd4bf]">
          ...
          <p className="text-[11px] text-[#7c8d8e]">Files upload to Supabase Storage bucket <code className="font-mono">corrections</code>. If missing, attachments are skipped.</p>
